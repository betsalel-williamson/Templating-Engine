 ...
 */
function convertObjectToDataContext(obj: unknown): DataContext {
...
const recipeV2Templates = fs
  .readdirSync(recipesDir)
  .filter((file) => file.endsWith('.v2.template'));

describe('Recipe V2 Golden File Tests', () => {
  const evaluate = createTestEvaluator(new Map(), false, 'new', false);

  for (const templateFile of recipeV2Templates) {
    const baseName = path.basename(templateFile, '.v2.template');
    ...
    if (!fs.existsSync(dataPath) || !fs.existsSync(resultPath)) {
      it.skip(`Recipe V2: ${baseName} (missing data or result file)`, () => {});
      continue;
    }

    describe(`Recipe V2: ${baseName}`, () => {
      it('should produce an output that matches the legacy golden file (.result)', async () => {
