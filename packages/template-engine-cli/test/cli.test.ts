...
describe('CLI Interface (Story 12)', () => {
  ...
  it('should return 1 and provide detailed location info for a malformed tag', async () => {
    ...
    expect(stderrOutput).toContain('Error: Expected end of input but "<" found.');
    expect(stderrOutput).toContain('template.txt:2:12');
    expect(stderrOutput).toContain('This is an <#malformed');
    expect(stderrOutput).toContain('^');
  });
});
