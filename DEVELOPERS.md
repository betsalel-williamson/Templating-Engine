...
| `DOGFOOD_SKILL`          | Treatment skill under `evals/dogfood/skills/` (default `v2-engine-build`)          |
...
### App codegen experiment

Calculator CLI A/B (legacy templates vs plain TS):

```bash
bash -lc 'DOGFOOD_TASK=calculator-cli DOGFOOD_SKILL=legacy-template-codegen DOGFOOD_KEEP_WORKTREES=1 pnpm dogfood:run'
```

See [`evals/dogfood/README.md`](evals/dogfood/README.md#app-codegen-pilot-calculator-cli).
