...
proc handle_rules_buf {rules_buf} {
    ...
    while {[string length $rules_buf] > 0} {
        ...
        if [regexp -- {\A<([^>]+)>} $pattern foo state_name] {
            ...
            if {$state_name != "*"} {
		foreach x [split $state_name ,] {
		    if { ![info exists ::state_table($x)]} {
			fickle_error "Undeclared start state $x" $::GRAMMAR_ERROR
		    }
		}
	    }
        }
    ...
    foreach rule $::rule_table {
        ...
        if $::startstates {
            if {$state_name == ""} {
                ...
                puts -nonewline $::dest "\[lsearch -exact \{[split $state_name ,]\} \$${::p}_current_state\] >= 0 && \\\n                "
            }
        }
